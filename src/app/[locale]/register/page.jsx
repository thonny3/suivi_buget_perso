...
// Composant d'icône SVG pour Google
const GoogleIcon = () => (
...
const CustomSelect = ({ value, onChange, options, placeholder, icon: Icon, error }) => {
  ...
  return (
    <div className="relative">
      ...
      {isOpen && (
        <div className="absolute z-10 w-full mt-1 bg-white border border-gray-300 rounded-lg shadow-lg max-h-60 overflow-auto custom-scrollbar scroll-smooth">
          {options.map((option) => (
            <button
              key={option.value}
              type="button"
  ...
  return (
    <RedirectIfAuthenticated locale={locale}>
      <div className="min-h-screen flex items-center justify-center px-4 sm:px-6 py-6 sm:py-8">
        <div className="bg-white rounded-2xl shadow-2xl p-4 sm:p-6 w-full max-w-md animate-fade-in overflow-y-auto max-h-[95vh] custom-scrollbar scroll-smooth">
        <button
          onClick={returnHome}

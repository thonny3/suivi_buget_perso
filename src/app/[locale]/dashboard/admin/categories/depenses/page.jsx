...
function AdminCategoriesDepensesContent({ locale }) {
  ...
  useEffect(() => {
    (async () => {
      try {
        const list = await adminCategories.listDepenses()
  ...
  const add = async () => {
    ...
    try {
      if (editId) {
        await adminCategories.updateDepense(editId, nom.trim())
        setItems(prev => prev.map(x => x.id === editId ? { ...x, nom: nom.trim() } : x))
      } else {
        const r = await adminCategories.addDepense(nom.trim())
  ...
  const confirmRemove = async () => {
    if (!deleteId) return
    try {
      await adminCategories.deleteDepense(deleteId)
  ...
  return (
    <div className="p-6">
      <div className="flex items-center mb-4">
        ...
        <h1 className="text-xl font-semibold">Catégories — Dépenses</h1>
      </div>
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-white rounded-2xl shadow-xl w/full max-w-md p-5">
            <div className="mb-3">
              <h2 className="text-lg font-semibold text-gray-900">{editId ? 'Modifier la catégorie' : 'Ajouter une catégorie dépense'}</h2>
              <p className="text-sm text-gray-500">Définissez un nom clair et descriptif.</p>
            </div>
            <div className="space-y-3">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Nom</label>
                <input value={nom} onChange={e=>setNom(e.target.value)} className="w-full border border-gray-300 rounded-lg px-3 py-2 outline-none focus:ring-2 focus:ring-emerald-500" placeholder="Ex: Logement, Transport…" />
              </div>
            </div>
            <div className="mt-5 flex justify-end gap-2">
...
export default function AdminCategoriesDepensesPage({ params }) {
  const { locale } = params
  
  return (
    <AdminProtectedRoute locale={locale}>
      <AdminCategoriesDepensesContent locale={locale} />
    </AdminProtectedRoute>
  )
}
